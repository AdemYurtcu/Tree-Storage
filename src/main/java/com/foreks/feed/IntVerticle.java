...
public class IntVerticle extends AbstractVerticle {
    ...
    public final void start() throws Exception {
        this.vertx.eventBus()
        .<String> consumer("Create.Integer")
        .handler(this::createInt);
        this.vertx.eventBus()
        .<String> consumer("Add.Integer")
        .handler(this::addInt);
        this.vertx.eventBus()
        .<String> consumer("Delete.Integer")
        .handler(this::deleteInt);
        this.vertx.eventBus()
        .<String> consumer("Update.Integer")
        .handler(this::updateInt);
        this.vertx.eventBus()
        .<String> consumer("Print.Integer")
        .handler(this::print);
        this.vertx.eventBus()
        .<String> consumer("Size.Integer")
        .handler(this::size);
    }
    ...
    public final void updateInt(final Message<String> m) {
        ...
        try {
            final TreeRequest studentTree = mapper.readValue(m.body(),
                                                             TreeRequest.class);
            ...
            m.reply(studentTree.getTreeId()
                    + " anahtarli agacin degerleri: "
                    + tree.toString());
    ...
    public final void createInt(final Message<String> m) {
        ...
        try {
            final TreeRequest studentTree = mapper.readValue(m.body(),
                                                             TreeRequest.class);
            final MyTreeSet<Integer> tree = new MyTreeSetImpl<Integer>(studentTree.getStrategy(),
                    this.comparator);
            this.intMap.put(this.a, tree);
            m.reply(this.a
                    + " anahtarli "
                    + studentTree.getStrategy()
                    + " Integer agac olusturuldu.");
        } catch (final IOException e) {
            e.printStackTrace();
        }
